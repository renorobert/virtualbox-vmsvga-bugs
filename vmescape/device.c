...
extern VBoxGuestPortInfo portinfo;
extern SVGADevice gSVGA;

/* some publicly available wrappers for SVGA device */
...
conf_vmm_device(void)
{
	...
	match.vendor_id = 0x80ee;
	match.device_id = 0xcafe;
	match.subvendor_id = PCI_MATCH_ANY;
	match.subdevice_id = PCI_MATCH_ANY;
	match.device_class = 0;
	match.device_class_mask = 0;
	...
	portinfo.portAddress = dev->regions[0].base_addr;
